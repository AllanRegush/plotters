...
- Plotters now moved all backend code to separate repository, check [FAQ list](#faq-list) for details
...
* How to check if a backend writes file successfully ?
	...
	whenever is needed. When the `present` method is called from `drop`, any error will be sliently ignored. 
